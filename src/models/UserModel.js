...
export default class UserModel {
    ...
    #validatePassword = (password, rPassword) => {
        ...
        let regex = new RegExp(/(?=.*[a-z])(?=.*[A-Z])(?=.*\d)([a-zA-Z\d!#$%@]+){6,}/g);
    
        if(rPassword == null || password !== rPassword || regex.exec(password) == null)
            throw exceptionHandler.validationException("Repetição de senha incompatível");
    }

    #validateUser = (user, full) => {
        
        if(user.email == null || typeof user.email == undefined/*|| !user.email.match('^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$')*/)
            throw exceptionHandler.validationException("E-mail inválido");
        
        if(user.password == null || user.password.length < 6)
            throw exceptionHandler.validationException("Senha não atende os requisitos mínimos");
       
        if(full){

            if(user.name == null || user.name.length < 5){
                throw exceptionHandler.validationException("Nome inválido");
            }
    ...
    signin = async (user) => {
        ...
        try{
          ...
          throw exceptionHandler.ErrorSendingEmailException(user.email);
   ...
   login = async (user) => {
        ...
        if(!userFound)
            throw exceptionHandler.validationException("E-mail incorreto");
  
        if(!this.#compare( userFound.password, user.password))
            throw exceptionHandler.validationException("Senha incorreta");
